...
// LIVE-PRODUCTION-FINAL v2 enhance
(function(){
  const isHome = () => location.pathname.endsWith('index.html') || location.pathname==='/' || location.pathname.includes('/loy-krathong-online/');
  document.addEventListener('DOMContentLoaded', function(){
    const h=new Date().getHours(), r=document.documentElement;
  ...
  function patchSave(){
    const S = window.Storage; if(!S || !S.save || S.__v2) return; const o=S.save.bind(S);
    S.save = async function(item){
      try{
        const name=(document.getElementById('name')?.value||'').trim().slice(0,50);
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderList(list){
      for (const it of list){
        const key = it.id || (String(it.ts)+':'+(it.text||''));
        if (rendered.has(key)) continue; rendered.add(key);
    ...
    if (typeof window.Storage.getAllDetailed==='function') window.Storage.getAllDetailed().then(l=>renderList(l.slice(-150))).catch(()=>{});
    if (typeof window.Storage.subscribeAll==='function'){
      window.__lk_unsub = window.Storage.subscribeAll(renderList);
      window.addEventListener('beforeunload', ()=>{ if (typeof window.__lk_unsub==='function') window.__lk_unsub(); });
    }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    const fx = window.fireworks || window.startFireworks || window.initFireworks;
    try{ if (typeof fx === 'function') fx(); }catch(e){}
    function playAudio(){ try{ const a=new Audio('assets/sound/fireworks-soft.wav'); a.volume=0.22; a.play().catch(()=>{});}catch(e){} }
    const unlock=()=>{ playAudio(); window.removeEventListener('pointerdown',unlock); window.removeEventListener('keydown',unlock);};
    window.addEventListener('pointerdown',unlock,{once:true}); window.addEventListener('keydown',unlock,{once:true});
    const btn=document.getElementById('launchBtn'); if (btn) btn.addEventListener('click', ()=>{ playAudio(); }, {passive:true});
  });
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    const id='lk_text_style_v2'; if(document.getElementById(id)) return;
    const s=document.createElement('style'); s.id=id; s.textContent='.krathong .text{color:#fff!important;font-weight:700;text-shadow:0 1px 3px rgba(0,0,0,.7);font-family:"Sarabun","Noto Sans Thai",system-ui,sans-serif}';
    document.head.appendChild(s);
  });
})();